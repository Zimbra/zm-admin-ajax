...
public class Forward extends ZCServlet {
    ...
    public void doGet(HttpServletRequest req,
        ...
        try {
            ...
            if (qs != null && !qs.equals("")) {
                url = url + "?" + qs;
            }            //encode url with query
            ServletContext sc = getServletConfig().getServletContext();
            sc.getRequestDispatcher(url).forward(req, resp);
        } catch (SecurityException ex) {
            ZimbraLog.webclient.warn("fu param did not match zmlocalconfig zimbra_forward_servlet_allowed_paths", ex);
            if (!resp.isCommitted()) {
    ...
    protected String getReqParameter(HttpServletRequest req, String paramName, String defaultValue) {
        ...
        if (PARAM_FORWARD_URL.equals(paramName) && LC.zimbra_forward_servlet_url_validation_enabled.booleanValue()
                && !isAllowedForwardUrl(val)) {
